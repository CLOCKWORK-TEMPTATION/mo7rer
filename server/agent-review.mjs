...
const normalizeReviewContextLine = (line, index) => {
    if (!isObjectRecord(line)) {
        throw new AgentReviewValidationError(`Invalid context line at index ${index}.`);
    }
    ...
    if (!isIntegerNumber(lineIndex)) {
        throw new AgentReviewValidationError(`Invalid context lineIndex at index ${index}.`);
    }
    if (!ALLOWED_LINE_TYPES.has(assignedType)) {
        throw new AgentReviewValidationError(`Invalid context assignedType at index ${index}.`);
    }
    if (!text) {
        throw new AgentReviewValidationError(`Empty context text at index ${index}.`);
    }
    return {
        lineIndex,
        assignedType,
...
const normalizeSuspiciousLine = (entry, index) => {
    if (!isObjectRecord(entry)) {
        throw new AgentReviewValidationError(`Invalid suspicious line payload at index ${index}.`);
    }
    ...
    const contextLines = Array.isArray(entry.contextLines)
        ? entry.contextLines.map((line, ctxIndex) => normalizeReviewContextLine(line, ctxIndex))
        : [];
    if (!isIntegerNumber(itemIndex)) {
        throw new AgentReviewValidationError(`Invalid itemIndex at suspicious line ${index}.`);
    }
    if (!isIntegerNumber(lineIndex)) {
        throw new AgentReviewValidationError(`Invalid lineIndex at suspicious line ${index}.`);
    }
    if (!text) {
        throw new AgentReviewValidationError(`Empty text at suspicious line ${index}.`);
    }
    if (!ALLOWED_LINE_TYPES.has(assignedType)) {
        throw new AgentReviewValidationError(`Invalid assignedType at suspicious line ${index}.`);
    }
    if (!isFiniteNumber(totalSuspicion) || totalSuspicion < 0 || totalSuspicion > 100) {
        throw new AgentReviewValidationError(`Invalid totalSuspicion at suspicious line ${index}.`);
    }
    return {
        itemIndex,
        lineIndex,
...
export const validateAgentReviewRequestBody = (rawBody) => {
    ...
    const suspiciousLines = Array.isArray(rawBody.suspiciousLines) ? rawBody.suspiciousLines : null;
    const reviewPacketText = normalizeIncomingText(rawBody.reviewPacketText, 120_000);
    if (!sessionId) {
        throw new AgentReviewValidationError("Missing sessionId in agent-review request.");
    }
    if (!isIntegerNumber(totalReviewed)) {
        throw new AgentReviewValidationError("Invalid totalReviewed in agent-review request.");
    }
    if (!Array.isArray(suspiciousLines)) {
        throw new AgentReviewValidationError("Invalid suspiciousLines in agent-review request.");
    }
    return {
        sessionId,
        totalReviewed,
        reviewPacketText: reviewPacketText || undefined,
        suspiciousLines: suspiciousLines.map((entry, index) => normalizeSuspiciousLine(entry, index)),
    };
};
...
const parseReviewDecisions = (rawText) => {
    ...
    const parseCandidate = (candidate) => {
        ...
        for (const decision of decisions) {
            ...
            const itemIndex = typeof record.itemIndex === "number"
                ? Math.trunc(record.itemIndex)
                : -1;
            const finalType = typeof record.finalType === "string" ? record.finalType.trim() : "";
            ...
            if (itemIndex < 0)
                continue;
            if (!ALLOWED_LINE_TYPES.has(finalType))
                continue;
            normalized.push({
                itemIndex,
                finalType: finalType,
...
export class ScreenplayClassifier {
    ...
    async convertDocToText(docPath) {
        ...
        try {
            const { stdout, stderr } = await execAsync(`wsl --exec antiword "${wslPath}"`, {
                encoding: "utf-8",
                timeout: 30000,
            });
            if (stderr) {
    ...
    _preprocessText(text) {
        ...
        const isSeparatorLine = (s) => {
            ...
            return (uniqueChars.size === 1 && (uniqueChars.has("=") || uniqueChars.has("-")));
        };
...
${text}`;
    ...
    async _classifyLongText(text, maxTokens, chunkSize) {
        ...
        for (let i = 0; i < lines.length; i += chunkSize) {
            ...
            try {
                ...
                console.warn(`تحذير: فشل SDK في الجزء ${chunkNum}، الرجوع إلى REST: ${error}`);
                const chunkResult = await this.classifyWithRestFallback(chunk, maxTokens);
                results.push(chunkResult);
            }
    ...
    async saveResult(result, sourceFile) {
        ...
        const timestamp = new Date()
            ...
            .replace(/[:T]/g, "_")
            .split(".")[0];
        const outputFile = path.join(this.outputDir, `${sourceName}_classified_${timestamp}.txt`);
        const header = [
...
export const reviewSuspiciousLinesWithClaude = async (request) => {
    ...
    if (!Array.isArray(request.suspiciousLines) ||
    ...
    try {
        ...
        const decisions = parseReviewDecisions(text).filter((decision) => request.suspiciousLines.some((line) => line.itemIndex === decision.itemIndex));
        ...
        console.warn(`تحذير: فشل SDK في المراجعة، تجربة REST fallback: ${sdkError}`);
        try {
            const apiKey = process.env.ANTHROPIC_API_KEY;
            if (!apiKey) {
