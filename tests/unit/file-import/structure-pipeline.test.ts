...
import { buildProjectionGuardReport, buildStructuredBlocksFromText, normalizeTextForStructure, segmentLinesStrict } from '../../../src/utils/file-import/structure-pipeline'
...
describe('structure-pipeline', () => {
  ...
  it('projection guard rejects destructive collapse against existing document', () => {
    const currentBlocks: ScreenplayBlock[] = new Array(14).fill(null).map((_, index) => ({
      formatId: index % 2 === 0 ? 'dialogue' : 'action',
      text: `line ${index}`,
    }))
    const nextBlocks: ScreenplayBlock[] = [{ formatId: 'action', text: 'collapsed' }]
