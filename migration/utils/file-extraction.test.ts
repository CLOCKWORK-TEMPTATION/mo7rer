...
describe("extractFileText", () => {
  ...
  it("extracts txt content", async () => {
    ...
    expect(result.structuredBlocks?.length).toBeGreaterThanOrEqual(2);
  });
  ...
  it("keeps non-payload docx text raw and builds strict structured blocks", async () => {
    mammothExtractRawTextMock.mockResolvedValueOnce({
    ...
    expect(result.structuredBlocks?.length).toBe(1);
    expect(result.structuredBlocks?.[0]?.formatId).toBe("scene-header-top-line");
  ...
  it("uses pdf-converter-flow TXT output for pdf import", async () => {
    ...
    expect(result.structuredBlocks?.length).toBeGreaterThanOrEqual(1);
  });
