...
export type PasteReviewProfile = "interactive" | "silent-live";

export interface HandlePasteOptions {
  reviewProfile?: PasteReviewProfile;
...
export const handlePaste = async (
  ...
  const reviewProfile = options.reviewProfile ?? "interactive";
  const mergePolicy = options.mergePolicy ?? "safe";
  const classifierRole = options.classifierRole ?? "limited-rewrite";
  const strictNoMerge =
    mergePolicy === "none" || classifierRole === "label-only";
  const allowInteractiveReview = reviewProfile === "interactive" && !strictNoMerge;
  const effectiveConfirmationCallback = allowInteractiveReview
    ? confirmationCallback
    : null;
  const effectivePendingConfirmations = allowInteractiveReview
    ? onPendingConfirmations
    : null;
  ...
  for (const rawLine of rawLines) {
    ...
    for (const piece of expanded) {
      ...
      if (lastIndex >= 0 && importSource === "clipboard" && !strictNoMerge) {
  ...
  const pendingCount = effectiveConfirmationCallback
  ...
  const runDeferredConfirmations = async (): Promise<void> => {
    ...
    if (hasConfirmations && effectiveConfirmationCallback) {
      for (let itemIndex = 0; itemIndex < collectedItems.length; itemIndex++) {
        const item = collectedItems[itemIndex];
        if (!item.needsConfirmation) continue;

        const suggested = item.suggestedType ?? item.classification;
        const confirmedType = await effectiveConfirmationCallback(
          item.line,
          suggested,
          item.confidence
        );
    ...
    if (allowInteractiveReview) {
      ...
      onAgentSkipped?.("profile-silent-live");
    }
    ...
    for (let itemIndex = 0; itemIndex < collectedItems.length; itemIndex++) {
      ...
      const formatClass =
        strictNoMerge || item.skipAutoFix
          ? resolvedType
  ...
  if (pendingCount > 0 && effectiveConfirmationCallback) {
    pendingPasteConfirmationJobs.set(pasteBatchId, {
      pendingCount,
      run: runDeferredConfirmations,
    });
    effectivePendingConfirmations?.(pasteBatchId, pendingCount);
  } else {
    setTimeout(() => {
