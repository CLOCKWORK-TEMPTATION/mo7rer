/**
 * @fileoverview agent-review.ts - أنواع بيانات مراجعة الوكيل الذكي (Agent Review)
 * 
 * @description
 * بيحدد كل الـ types والـ interfaces اللي بيستخدمها نظام المراجعة الذكية.
 * النظام ده بيبعت سطور "مشتبهة" للـ AI (Claude) عشان يراجعها ويحدد التصنيف الصحيح.
 * 
 * @system Agent Review System
 * 1. الكشف عن سطور مشتبهة (بناءً على sequence violations, split fragments, إلخ)
 * 2. بناء سياق (context) للسطور المحيطة
 * 3. إرسال للـ AI للمراجعة
 * 4. استلام القرارات وتطبيقها
 * 
 * @exports
 * - AgentReviewContextLine: سطر ضمن السياق المرسل للوكيل
 * - AgentSuspiciousLinePayload: سطر مشتبه تم اكتشافه
 * - AgentReviewRequestPayload: طلب المراجعة الكامل
 * - AgentReviewDecision: قرار الوكيل لسطر واحد
 * - AgentReviewResponsePayload: استجابة الوكيل النهائية
 * 
 * @usage
 * import { AgentReviewRequestPayload, AgentReviewDecision } from "@/types/agent-review";
 * 
 * @author أفان تيتر
 * @version 1.0.0
 */
