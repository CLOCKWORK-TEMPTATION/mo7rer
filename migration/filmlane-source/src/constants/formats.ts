/**
 * @fileoverview formats.ts - تنسيقات السيناريو
 * 
 * @description
 * بيحدد كل تنسيقات أسطر السيناريو (scene headers, action, character, dialogue, إلخ)
 * مع الاختصارات، الألوان، والأيقونات المرتبطة بكل تنسيق. كمان بيوفر maps للوصول
 * السريع للتنسيقات وclassification options.
 * 
 * @exports
 * - screenplayFormats: Array of all screenplay format definitions
 * - formatClassMap: Mapping from format ID to CSS class name
 * - formatShortcutMap: Keyboard shortcuts (Ctrl+1, Ctrl+2, إلخ)
 * - classificationTypeOptions: Options for the confirmation dialog
 * 
 * @formats
 * - basmala: بسملة
 * - scene-header-1/2/3: عناوين المشاهد
 * - action: الوصف/الحدث
 * - character: شخصية
 ...
 * const actionFormat = screenplayFormats.find(f => f.id === "action");
 * 
 * @author أفان تيتر
 * @version 1.0.0
 */
