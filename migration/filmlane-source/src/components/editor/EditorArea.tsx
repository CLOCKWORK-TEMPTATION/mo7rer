/**
 * @fileoverview EditorArea.tsx - منطقة التحرير الفعلية
 * 
 * @description
 * المكون الرئيسي للتحرير. بيدير contentEditable divs، pagination، clipboard operations،
 * undo/redo، والتصنيف الذكي باستخدام AI. بيدعم وضعين: legacy (native) و tiptap.
 * 
 * @features
 * - ✏️ تحرير فعلي باستخدام contentEditable
 * - 📄 نظام صفحات تلقائي (repagination)
 * - 📋 Clipboard متقدم (copy/cut/paste مع الحفاظ على التنسيق)
 * - ↩️ Undo/Redo تاريخ الأوامر
 * - 🤖 تصنيف ذكي للأسطر باستخدام HybridClassifier
 * - ✅ حوار تأكيد للتصنيفات غير المؤكدة
 * 
 * @usage
 * بيستخدم داخل ScreenplayEditor مع forwardRef عشان يوصل الـ EditorHandle API.
 * 
 * @example
 * <EditorArea
 *   ref={editorRef}
...
import { ScreenplayTiptapEditor } from "./tiptap/ScreenplayTiptapEditor";
import {
  formatClassMap,
  screenplayFormats,
  formatShortcutMap,
...
import type { EditorAreaProps, EditorHandle } from "./editor-area.types";
  ...
  (
    ...
    if (engineMode === "tiptap") {
      return (
        <ScreenplayTiptapEditor
          ...
          size={_size}
          pageCount={_pageCount}
          onImporterReady={onImporterReady}
    ...
    const applyFormatToCurrentLine = (formatType: string) => {
      ...
      if (currentElement && currentElement instanceof HTMLElement) {
        ...
        const nextClassName = formatClassMap[formatType] ?? formatClassMap.action;
        currentElement.classList.add(nextClassName);
        const styles = getFormatStyles(formatType, fixedSize, fixedFont);
        currentElement.setAttribute("style", cssObjectToString(styles));
        currentElement.setAttribute("dir", "rtl");
    ...
    const importViaPastePipeline = useCallback(
      async (
        ...
        reviewProfile: PasteWorkflowReviewProfile = "interactive"
      ) => {
        ...
        const interactiveReview = reviewProfile === "interactive";
        ...
        await newHandlePaste(
          ...
          hybridClassifier,
          feedbackCollector,
          interactiveReview ? requestConfirmation : null,
          interactiveReview
    ...
    const runDocumentThroughPasteWorkflow = useCallback(
      async (
        ...
        const fullText = getAllContentNodes()
    ...
    const resolveActiveLineAfterEnter = useCallback(
      (range: Range): HTMLDivElement | null => {
        const directContainer = range.startContainer;

        if (directContainer.nodeType === Node.ELEMENT_NODE) {
          const element = directContainer as HTMLElement;
          if (element.classList.contains("screenplay-sheet__body")) {
            ...
            if (
              fromOffset &&
              fromOffset.nodeType === Node.ELEMENT_NODE &&
              (fromOffset as HTMLElement).tagName === "DIV"
            ) {
              return fromOffset as HTMLDivElement;
            }
          }
        }

        let element: HTMLElement | null =
          directContainer.nodeType === Node.ELEMENT_NODE
            ? (directContainer as HTMLElement)
        ...
        while (element) {
          ...
          if (element.classList.contains("screenplay-sheet__body")) {
            const fromEnd = element.lastElementChild;
            if (fromEnd && fromEnd.tagName === "DIV") {
              return fromEnd as HTMLDivElement;
            }
            return null;
          }
    ...
    const applyFormatToLineElement = useCallback(
      (lineElement: HTMLElement, nextFormat: string) => {
        Object.values(formatClassMap).forEach((cls) => {
          lineElement.classList.remove(cls);
        });
        const nextClassName = formatClassMap[nextFormat] ?? formatClassMap.action;
        lineElement.classList.add(nextClassName);
        const styles = getFormatStyles(nextFormat, fixedSize, fixedFont);
        lineElement.setAttribute("style", cssObjectToString(styles));
        lineElement.setAttribute("dir", "rtl");
      },
      [fixedFont, fixedSize]
    );
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === "Enter" && !e.shiftKey) {
        ...
        const transition = resolveTypingOnEnter(currentFormat);
        const nextFormat = transition.nextFormat;
        ...
        if (selection && selection.rangeCount > 0) {
          const range = selection.getRangeAt(0);
          const lineElement = resolveActiveLineAfterEnter(range);
          if (lineElement) {
            applyFormatToLineElement(lineElement, nextFormat);
            if (transition.seedText) {
              lineElement.textContent = transition.seedText;
            }

            const caretRange = document.createRange();
            caretRange.selectNodeContents(lineElement);
      ...
      if (e.key === "Tab") {
        ...
        const transition = resolveTypingOnTab(currentFormat, {
          isEmpty,
...
export type { EditorAreaProps, EditorHandle } from "./editor-area.types";
