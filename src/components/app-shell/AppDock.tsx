...
function DockIconButton({
  ...
  return (
    <div className={`relative z-10 flex items-center justify-center ${isMobile ? 'h-9 w-9' : 'h-10 w-10'}`}>
      <HoverBorderGradient
        as="button"
        onClick={onClick}
        title={title}
        ...
        duration={1}
      >
        <Icon className={isMobile ? 'size-4' : 'size-[18px]'} strokeWidth={1.75} />
      </HoverBorderGradient>
    </div>
  )
}

export function AppDock({ buttons, onAction, isMobile }: AppDockProps): React.JSX.Element {
  const visibleButtons = isMobile
    ? buttons.filter((_, index) => [0, 1, 6, 7, 8, 9, 10, 11, 14].includes(index))
    : buttons
      ...
      <div className="pointer-events-auto">
        ...
        >
          {visibleButtons.map((button, index) => {
            const showSeparator = !isMobile && (index === 1 || index === 3 || index === 7 || index === 13)
            return (
