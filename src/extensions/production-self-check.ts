...
const EXTENSION_FUNCTION_CALL_COUNT = 66;
...
const runExtensionFunctionCalls = async (sampleText: string): Promise<void> => {
  ...
  const agent = new ScreenplayClassifier(MODEL_ID);
  agent.getModel();
  await agent.classify(sampleText);
  try {
    await agent.processFile("unsupported");
  } catch {
    // expected in browser runtime.
  }
